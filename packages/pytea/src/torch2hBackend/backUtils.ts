...
export function trackMro(value: ShValue, heap: ShHeap, env: ShEnv): (number | undefined)[] {
    ...
    switch (obj.type) {
        case SVType.Bool:
            return [(sanitizeAddr(heap, env.getId('bool')) as SVAddr | undefined)?.addr, objectAddr];
        case SVType.Int:
            return [(sanitizeAddr(heap, env.getId('int')) as SVAddr | undefined)?.addr, objectAddr];
        case SVType.Float:
            return [(sanitizeAddr(heap, env.getId('float')) as SVAddr | undefined)?.addr, objectAddr];
        case SVType.String:
            return [(sanitizeAddr(heap, env.getId('str')) as SVAddr | undefined)?.addr, objectAddr];
        case SVType.Object:
            break;
        default:
            return [objectAddr];
    ...
    let clsAddr: ShValue | undefined = sanitizeAddr(heap, mro.getIndice(0));
    while (clsAddr?.type === SVType.Addr) {
        ...
        if (mro?.type !== SVType.Object) break;
        clsAddr = sanitizeAddr(heap, mro.getIndice(1));
    }
